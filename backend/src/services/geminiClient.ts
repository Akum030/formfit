...
- When form is bad (score < 50), give ONE clear correction.
...
function buildPrompt(ctx: CoachingContext): string {
  ...
  let prompt = `[STATE] ${ctx.exerciseName} | Score: ${ctx.currentScore}% (${scoreLabel}) | Rep ${ctx.repCount}/${ctx.targetReps} | Set ${ctx.setNumber}/${ctx.targetSets}`;
...
function getFallbackFeedback(ctx: CoachingContext): string {
  ...
  if (ctx.userTranscript) {
    ...
    if (t.includes('how many') && (t.includes('rep') || t.includes('done'))) {
      return `You have done ${ctx.repCount} reps out of ${ctx.targetReps}. Keep pushing!`;
    }
    if (t.includes('score') || t.includes('form') || t.includes('how am i')) {
      const label = ctx.currentScore >= 80 ? 'great' : ctx.currentScore >= 50 ? 'decent, small fixes needed' : 'needs some work';
      return `Your form is ${label} at ${ctx.currentScore}%. ${ctx.currentIssues.length > 0 ? 'Focus on: ' + ctx.currentIssues[0] : 'Keep it up!'}`;
    }
    if (t.includes('what') && (t.includes('wrong') || t.includes('fix') || t.includes('improve'))) {
      return ctx.currentIssues.length > 0
        ? `Main thing to fix: ${ctx.currentIssues[0]}. You got this!`
        : 'Your form looks solid right now! Keep that up!';
    }
    if (t.includes('set') || t.includes('how many set')) {
      return `You are on set ${ctx.setNumber} of ${ctx.targetSets}. ${ctx.targetSets - ctx.setNumber > 0 ? (ctx.targetSets - ctx.setNumber) + ' sets to go!' : 'This is your last set, finish strong!'}`;
    }
    if (t.includes('rest') || t.includes('break') || t.includes('tired')) {
      return 'Take a quick breather if you need it. Come back strong for the next rep!';
    }
    if (t.includes('hello') || t.includes('hi') || t.includes('hey')) {
      return `Hey! I am your coach. You are doing ${ctx.exerciseName}. Let us crush it!`;
    }
    // Generic contextual responses that at least reference the exercise
    const responses = [
    ...
    return responses[Math.floor(r * responses.length)];
  }
  ...
  if (ctx.currentScore >= 50) {
    if (ctx.currentIssues.length > 0) {
      return `Watch your form — ${ctx.currentIssues[0]}`;
    }
    const pool = [
  ...
  if (ctx.currentIssues.length > 0) {
    return `Focus on this: ${ctx.currentIssues[0]}`;
  }
  return 'Slow down and focus on your form. Quality over speed!';
}
